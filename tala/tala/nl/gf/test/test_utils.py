...
class str_phrase_test_case(unittest.TestCase):
    def test_base_case(self):
        self.assert_phrase_to_string(["calling", "alex", "berman"], "calling alex berman")

    def test_closing_punctuation(self):
        self.assert_phrase_to_string(["okay", "."], "okay.")
        self.assert_phrase_to_string(["okay", "?"], "okay?")
        self.assert_phrase_to_string(["okay", "!"], "okay!")

    def test_comma(self):
        self.assert_phrase_to_string(["okay", ",", "so"], "okay, so")

    def test_bind_genitive(self):
        self.assert_phrase_to_string(["berman", utils.BIND, "'", utils.BIND, "s"], "berman's")

    def test_bind_between_word_and_comma(self):
        self.assert_phrase_to_string(["berman", utils.BIND, ",", "alex"], "berman, alex")

    def assert_phrase_to_string(self, tokens, expected_string):
        self.assertEqual(expected_string, utils.str_phrase(tokens))


class GFTests(unittest.TestCase):
    def test_tokenise(self):
        self.assertEqual(['pred', '(', 'arg', ')'], utils.tokenise('pred(arg)'))

    def test_tokenise_with_string_constant(self):
        self.assertEqual(['you', 'said', '_placeholder_0_', '.'], utils.tokenise('you said _placeholder_0_.'))

    def test_tokenise_genitive(self):
        self.assertEqual(["Alex's"], utils.tokenise("Alex's"))

    def test_tokenise_contraction(self):
        self.assertEqual(["don't"], utils.tokenise("don't"))

    def test_tokenise_time_expressions(self):
        self.assertEqual(["1:30", "10:30"], utils.tokenise("1:30 10:30"))

    def test_tokenise_time_expressions_negative(self):
        self.assertEqual([":", "30", "10:30", "3", "101", ":", "30", "19", ":", "1"],
                         utils.tokenise(":30 10:303 101:30 19:1"))

    def test_tokenise_with_magic_wording(self):
        self.assertEqual(["a", "message"], utils.tokenise("a message"))

    def test_tokenise_am_pm(self):
        self.assertEqual(["p.m.", "2:10", "a.m."], utils.tokenise("p.m. 2:10 a.m."))

    def test_tokenise_am_pm_and_magic_wordings(self):
        self.assertEqual(["send", "a", "message", "at", "2", "p.m."], utils.tokenise("send a message at 2 p.m."))

    def test_tokenisation_doesnt_split_at_hyphen(self):
        self.assertEqual(["Berlin-Tegel"], utils.tokenise("Berlin-Tegel"))

    def test_dont_split_7bit_encoded_tokens(self):
        string = "ställa klockan"
        expected_tokens = ["ställa", "klockan"]
        self.assertEqual(expected_tokens, utils.tokenise(string))
