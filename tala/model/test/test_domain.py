...
class DomainTests(LibTestCase):
    ...
    def _create_semantic_objects(self):
        ...
        self.dest_country_question = WhQuestion(
            LambdaAbstractedPredicateProposition(self.predicate_dest_country,
                                                 self.ontology_name))
    ...
    def _create_domains(self):
        ...
        dependencies = {
            self.dest_country_question: {self.dest_city_question}
            }

        self.non_verbalized_question = self.dest_city_question

        parameters = {
            self.non_verbalized_question: {"verbalize": False},
        }

        self.domain = Domain(self.DDD_NAME,
                             self.domain_name,
                             self.ontology,
                             plans=plans,
            ...
            {"goal": PerformGoal(self.downdate_plan_true_action),
             "plan": Plan([
                 InvokeServiceActionPlanItem(
                     "mockup_ontology", "mock_service_action", downdate_plan=True)]),
            },

            {"goal": PerformGoal(self.downdate_plan_false_action),
             "plan": Plan([
                 InvokeServiceActionPlanItem(
                     "mockup_ontology", "mock_service_action", downdate_plan=False)]),
            },
    ...
    def test_has_goal_true_for_question_with_plan(self):
        self.assertTrue(
            self.domain.has_goal(ResolveGoal(self.price_question, Speaker.SYS)))
    ...
    def test_goal_is_preferred_true_if_plan_always_preferred(self):
        self.assertTrue(self.domain.goal_is_preferred(
    ...
    def test_goal_is_preferred_false_if_plan_never_preferred(self):
        ...
        self.assertFalse(self.domain.goal_is_preferred(
            PerformGoal(never_preferred_action)))
    ...
    def test_get_plan_questions(self):
        self.maxDiff = None
        expected_questions = ["?X.dest_city(X)",
                              "?X.dest_city_type(X)",
                              "?X.dept_city(X)",
                              "?X.price(X)"]
        actual_result = [question for question in
                          self.domain.get_plan_questions()]
    ...
    def test_get_dependent_question_returns_none_for_goal_issue(self):
        self.assertEquals(None, self.domain.get_dependent_question(
                self.price_question))
    ...
    def test_get_postconds_for_goal_with_postcond(self):
        ...
        self.assertEqual([self.condition], list(self.domain.get_postconds(
            PerformGoal(action_with_postconds))))

    def test_get_postconds_for_goal_without_postconds(self):
        action_without_postcond = self.always_preferred_action
        self.assertEqual([], list(self.domain.get_postconds(
            PerformGoal(action_without_postcond))))

    def test_get_postconds_returns_implicit_postconds_for_service_action_invocations_if_downdate_plan_is_true(self):
        self.assertEquals(
            [ServiceActionTerminatedProposition("mockup_ontology", "mock_service_action")],
    ...
    def test_get_postconds_doesnt_return_implicit_postconds_for_service_action_invocations_if_not_downdating_plan(self):
        self.assertEquals(
            [], list(self.domain.get_postconds(PerformGoal(self.downdate_plan_false_action))))

    def test_get_postconds_considers_downdate_plan_property_for_handle_goals(self):
        self.assertEquals(
            [ServiceActionTerminatedProposition("mockup_ontology", "event")],
    ...
    def test_get_superactions_for_goal_with_superactions(self):
        ...
        self.assertEqual(expected_result, self.domain.get_superactions(
            PerformGoal(action_with_superactions)))

    def test_get_superactions_for_goal_without_superactions(self):
        action_without_superactions = self.buy_action
        self.assertEqual([], self.domain.get_superactions(
            PerformGoal(action_without_superactions)))
    ...
    def test_is_depending_on_true_due_to_domain_declaration(self):
        self.assertTrue(self.domain.is_depending_on(self.dest_country_question,
                                                    self.dest_city_question))

    def test_is_depending_on_false_for_inverse_domain_declaration(self):
        self.assertFalse(self.domain.is_depending_on(self.dest_city_question,
                                                     self.dest_country_question))

    def test_is_depending_on_true_because_of_featurehood(self):
        self.assertTrue(self.domain.is_depending_on(self.dest_city_question,
                                                    self.dest_city_type_question))
    ...
    def test_get_gui_context_for_goal_without_context(self):
        ...
        self.assertEquals(expected_result, self.domain.get_gui_context(
            PerformGoal(goal_without_context)))

    def test_get_resolving_answers(self):
        question = self.dest_city_question
        expected_answers = [self.proposition_dest_city_paris,
                            self.proposition_dest_city_london]
    ...
    def test_get_question_verbalize_returns_false_when_specified_as_false(self):
        self.assertFalse(self.domain.get_verbalize(
                self.non_verbalized_question))

    def test_empty_top_plan_by_default(self):
        self.assertEquals(Plan([]), self.domain.get_plan(
            PerformGoal(self.top_action)))
    ...
    def test_get_questions_in_plan_base_case(self):
        ...
        actual_result = [question for question in
                          self.domain.get_questions_in_plan(plan)]
    ...
    def test_get_questions_in_plan_supports_if_then_else(self):
        ...
        plan = Plan([
            IfThenElse(arbitrary_condition,
                       FindoutPlanItem(self.domain_name, self.price_question),
...
class DominatesTests(LibTestCase):
    def setUp(self):
        ...
        actions = {
            "dominating_action",
            "dominated_action",
            "super_dominated_action",
            "non_dominated_action",
            "planless_action"
        }
        sorts = set([])
        predicates = {
        ...
        plans = [
            {"goal": PerformGoal(self.dominating_action),
             "plan": [self._findout_with_alts([GoalProposition(PerformGoal(self.dominated_action)),
                                               GoalProposition(ResolveGoal(
                                                   self.dominated_issue, Speaker.SYS))
                                               ])]},
    ...
    def test_dominates_true_for_dominated_action(self):
        self.assertTrue(self.domain.dominates(PerformGoal(self.dominating_action),
                                              PerformGoal(self.dominated_action)))

    def test_dominates_true_for_super_dominated_action(self):
        self.assertTrue(self.domain.dominates(PerformGoal(self.dominating_action),
                                              PerformGoal(self.super_dominated_action)))

    def test_dominates_true_for_dominated_issue(self):
        self.assertTrue(self.domain.dominates(PerformGoal(self.dominating_action),
                                              ResolveGoal(self.dominated_issue, Speaker.SYS)))

    def test_dominates_false_for_non_dominated_action(self):
        self.assertFalse(self.domain.dominates(PerformGoal(self.dominating_action),
                                               PerformGoal(self.non_dominated_action)))

    def test_dominates_false_for_non_dominated_issue(self):
        self.assertFalse(self.domain.dominates(PerformGoal(self.dominating_action),
                                               ResolveGoal(self.non_dominated_issue, Speaker.SYS)))

    def test_dominates_false_for_action_without_plan(self):
        self.assertFalse(self.domain.dominates(PerformGoal(self.planless_action),
                                               PerformGoal(self.dominated_action)))

    def _findout_with_alts(self, alts):
        return FindoutPlanItem(self.domain_name,
                               AltQuestion(PropositionSet(alts)))
...
class IOStatusTests(LibTestCase):
    ...
    def _create_domain(self):
        ...
        plans = [{"goal": PerformGoal(self.default_action),
                  "plan": []},
    ...
    def test_io_status_for_default_plan(self):
        self.assertEquals(Domain.DEFAULT_IO_STATUS,
                          self.domain.get_io_status(PerformGoal(self.default_action)))

    def test_io_status_for_excluded_plan(self):
        self.assertEquals(Domain.EXCLUDED_IO_STATUS,
                          self.domain.get_io_status(PerformGoal(self.excluded_action)))

    def test_io_status_for_hidden_plan(self):
        self.assertEquals(Domain.HIDDEN_IO_STATUS,
                          self.domain.get_io_status(PerformGoal(self.hidden_action)))

    def test_io_status_for_silent_plan(self):
        self.assertEquals(Domain.SILENT_IO_STATUS,
                          self.domain.get_io_status(PerformGoal(self.silent_action)))

    def test_io_status_for_disabled_plan(self):
        self.assertEquals(Domain.DISABLED_IO_STATUS,
                          self.domain.get_io_status(PerformGoal(self.disabled_action)))
