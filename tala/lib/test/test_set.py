...
class SetTests(unittest.TestCase, EqualityAssertionTestCaseMixin):
    def testSet(self):
        testset = tala.lib.set.Set()
        testset.add("first")
        testset.add("second")
        self.assertEquals(len(testset), 2)

    def test_ininitialize_with_content(self):
        testset = tala.lib.set.Set(["first", "second"])
        self.assertEquals(len(testset), 2)

    def testSetEq(self):
        testset1 = tala.lib.set.Set()
        testset1.add("first")
        testset1.add("second")
        testset2 = tala.lib.set.Set()
        testset2.add("second")
        testset2.add("first")
    ...
    def test_empty_set_is_subset_of_any_set(self):
        testset = tala.lib.set.Set()
        testset.add("first")
        self.assertTrue(tala.lib.set.Set().is_subset_of(testset))
    ...
    def test_single_elem_set_not_subset_of_empty_set(self):
        testset = tala.lib.set.Set()
        testset.add("first")
        self.assertFalse(testset.is_subset_of(tala.lib.set.Set()))
    ...
    def test_subset_false_for_sets_with_empty_intersection(self):
        first_set = tala.lib.set.Set()
        first_set.add("first")
        second_set = tala.lib.set.Set()
    ...
    def testTypedSet(self):
        testset = tala.lib.set.Set(str)
        testset.add("foo")

    def testTypedSetTypeError(self):
        testset = tala.lib.set.Set(int)
        exception_raised = False
        try:
            testset.add("foo")
        except TypeError:
    ...
    def test_remove_existing_element_from_single_element_set(self):
        set = tala.lib.set.Set(["first"])
        set.remove("first")
        self.assertEquals(0, len(set))

    def test_remove_existing_element(self):
        set = tala.lib.set.Set(["first", "second"])
        set.remove("first")
        expected_result = tala.lib.set.Set(["second"])
    ...
    def test_remove_if_exists_for_non_existing_element_has_no_effect(self):
        ...
        expected_result = tala.lib.set.Set(["first", "second"])
        self.assertEquals(expected_result, set)

    def test_union_with_empty_set(self):
        test_set = tala.lib.set.Set(["first", "second"])
        union = test_set.union(tala.lib.set.Set())
    ...
    def test_union_with_empty_set_reflexive(self):
        test_set = tala.lib.set.Set(["first", "second"])
        union = tala.lib.set.Set().union(test_set)
        self.assertEquals(test_set, union)

    def test_union_with_empty_set_reflexive(self):
        first_list = ["first", "second"]
        second_list = ["third", "fourth"]
        first_set = tala.lib.set.Set(first_list)
        second_set = tala.lib.set.Set(second_list)
    ...
    def test_union_is_new_instance(self):
        ...
        self.assertEquals(tala.lib.set.Set(), union_set)
    ...
    def test_intersection_with_common_elements(self):
        first_set = tala.lib.set.Set([1, 2])
        second_set = tala.lib.set.Set([2, 3])
        intersection_set = first_set.intersection(second_set)

        expected_intersection = tala.lib.set.Set([2])
        self.assertEquals(expected_intersection, intersection_set)

    def test_intersection_disjoint_sets(self):
        first_set = tala.lib.set.Set([1])
        second_set = tala.lib.set.Set([2])
        intersection_set = first_set.intersection(second_set)
        expected_intersection_set = tala.lib.set.Set()
        self.assertEquals(intersection_set, expected_intersection_set)

    def test_set_in_set(self):
        set = tala.lib.set.Set()
        set_in_set = tala.lib.set.Set()
        set.add(set_in_set)
    ...
    def test_extend(self):
        test_set = tala.lib.set.Set(["first", "second"])
        extension = tala.lib.set.Set(["second", "third"])
        test_set.extend(extension)
        expected_result = tala.lib.set.Set(["first", "second", "third"])
