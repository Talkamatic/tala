...
class DddPyCompilerTestCase(DddCompilerTestCase):
    ...
    def _compile_ontology(
        self, name="Ontology", sorts={}, predicates={}, individuals={}, actions=set()):
        class Ontology: pass
    ...
    def _when_compile_domain(self, **kwargs):
        ...
        self._result = DddPyCompiler().compile_domain(
            self.DDD_NAME, Domain, self._ontology, self._parser)

    def _when_compile_domain_with_plan(self, plan):
        self._when_compile_domain(
            plans=[{"goal": "perform(top)",
                    "plan": plan}])
    ...
    def _compile_grammar(self, string):
        ...
        self._result = self._grammar_compiler.compile(
            string, self._ontology, self._service_interface)
...
class OntologyCompilerTests(DddPyCompilerTestCase):
    ...
    def test_custom_sort(self):
        ...
        self._then_result_has_field(
            "sorts", set([CustomSort(self._ontology_name, "city")]))

    def test_dynamic_sort(self):
        self._when_compile_ontology(sorts = {"city": {"dynamic": True}})
        self._then_result_has_field(
            "sorts", set([CustomSort(self._ontology_name, "city", dynamic=True)]))
    ...
    def test_predicate_of_custom_sort(self):
        ...
        self._then_result_has_field(
            "predicates",
            set([self._predicate("dest_city", CustomSort(self._ontology_name, "city"))]))

    def test_predicate_of_builtin_sort(self):
        self._when_compile_ontology(
            predicates = {"price": "real"})
        self._then_result_has_field(
            "predicates",
    ...
    def test_predicate_of_dynamic_sort(self):
        self._when_compile_ontology(
            predicates = {"dest_city": "city"},
            sorts = {"city": {"dynamic": True}})
        self._then_result_has_field(
            "predicates",
            set([self._predicate("dest_city", CustomSort(self._ontology_name, "city", dynamic=True))]))
    ...
    def test_feature_of(self):
        ...
        self._then_result_has_field(
            ...
            set([self._predicate(
                        "dest_city",
                        sort=CustomSort(self._ontology_name, "city")),
                 self._predicate(
                        "dest_city_type",
                        sort=CustomSort(self._ontology_name, "city"),
    ...
    def test_individuals(self):
        self._when_compile_ontology(
            individuals = {"paris": "city"},
            sorts = {"city": {}})
        self._then_result_has_field(
            "individuals", {"paris": CustomSort(self._ontology_name, "city")})
...
class PlanCompilationTests(DddPyCompilerTestCase):
    ...
    def test_plan_for_action_yields_helpful_exception(self):
        ...
        self.assertTrue('"top" is not a goal. Perhaps you mean "perform(top)"?' in
    ...
    def test_plan_for_issue_yields_helpful_exception(self):
        ...
        self.assertTrue('"?X.price(X)" is not a goal. '
                          'Perhaps you mean "resolve(?X.price(X))"?' in
                        unicode(cm.exception))
    ...
    def test_postconds(self):
        self._given_compiled_ontology(
            predicates={"dest_city": "city"},
            sorts={"city": {}},
            individuals={"gothenburg": "city"})
        self._when_compile_plan_with_attribute(
            "postconds", ["dest_city(gothenburg)"])
        self._then_result_has_plan_with_attribute(
            "postconds", [
...
class DomainCompilerTests(DddPyCompilerTestCase):
    ...
    def test_dependency(self):
        ...
        self._when_compile_domain(
            dependencies = {
                "?X.dest_country(X)": ["?X.dest_city(X)"]
                })
    ...
    def test_parameters(self):
        ...
        self._when_compile_domain(
            parameters={"?X.price(X)": "{graphical_type=list}"})
    ...
    def test_predicate_parameters(self):
        ...
        self._when_compile_domain(
            parameters={"price": "{background=[dest_city]}"})

        self._then_result_has_field(
            "parameters",
            {self._parse("price"): {"background": [self._parse("dest_city")]}})
...
class GrammarCompilerTests(DddPyCompilerTestCase):
    ...
    def test_action_with_sortal_answer_of_custom_sort(self):
        ...
        self._then_grammar_is([
                        Node(Constants.ACTION, {"name": "buy"},
                             [Node(Constants.ITEM, {}, ["purchase trip to ",
                                              Node(Constants.SLOT, {"sort": "city"})])])])

    def test_action_with_sortal_string_answer(self):
        self._given_compiled_ontology(
            actions = set(["set_comment"]),
            predicates = {"comment": "string"})
        self._when_compile_grammar('set_comment = "tell the hotel that <answer:string>"')
        self._then_grammar_is([
                        Node(Constants.ACTION, {"name": "set_comment"},
                             [Node(Constants.ITEM, {}, ["tell the hotel that ",
                                              Node(Constants.SLOT, {"sort": "string"})])])])
    ...
    def test_system_question(self):
        ...
        self._then_grammar_is([
                Node(Constants.SYS_QUESTION, {"predicate": "price"},
                     [Node(Constants.ITEM, {}, ["price information"])])])
...
""")
    ...
    def test_answer_combination(self):
        self._given_compiled_ontology(
            ...
            predicates={"dest_city": "city",
                        "dept_city": "city"})
        self._when_compile_grammar('''ANSWER = ["from <answer:dept_city> to <answer:dest_city>",
    ...
    def test_decompile_key_with_single_arg(self):
        self.assertEquals(
            "price_user_question",
            GrammarCompiler()._decompile_key(
                Node(Constants.USER_QUESTION, {"predicate": "price"})))
    ...
    def test_decompile_node(self):
        self.assertEquals(
            ...
            GrammarCompiler().decompile_node(
                Node(Constants.ACTION, {'name': 'make_reservation'},
                     [u'make reservation'])))
