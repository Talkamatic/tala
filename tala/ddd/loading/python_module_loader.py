...
class MissingModuleException(Exception): pass
...
class PythonModuleLoader(object):
    ...
    def load_py_module_class(self, module_path, ParentClass):
        if os.path.exists(module_path):
            ...
            raise MissingModuleException("%s missing in DDD %r at %r" % (
                module_path, self._name, os.getcwd()))
    ...
    def _load_py_module(self, module_filename):
        ...
        try:
            ...
            raise DddLoaderException(
                "failed to load module %r from %s" % (module_filename, os.getcwd()))
    ...
    def _find_subclass_in_module(self, BaseClass, module):
        ...
        if not subclasses:
            raise DddLoaderException("Could not find class %s in file %r"
                                     % (BaseClass.__name__, os.path.abspath(module.__file__)))
        if len(subclasses) > 1:
            raise DddLoaderException("Expected one class with base class %r but found %s"
                                     % (BaseClass.__name__, unicodify(subclasses)))
    ...
    def _find_classes_defined_in_module(self, classes, module):
        ...
        return [class_
                for name, class_
                in classes
                if is_class_defined_in_module(class_, module)]
