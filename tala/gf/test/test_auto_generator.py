...
class AutoGeneratorTestCase(unittest.TestCase):
    ...
    def given_compulsory_grammar_for_app(self, exclude=None):
        entries = [
            (Constants.ACTION, {"name": "top"}, [Node(Constants.ITEM, {}, ["start view"])]),
            (Constants.ACTION, {"name": "up"}, [Node(Constants.ITEM, {}, ["go back"])]),
            (Constants.ACTION, {"name": "make_reservation"}, [Node(Constants.ITEM, {}, ["make a reservation"])]),
            (Constants.ACTION, {"name": "settings"}, [Node(Constants.ITEM, {}, ["settings"])]),
            (Constants.ACTION, {"name": "set_time"}, [Node(Constants.ITEM, {}, ["set the time"])]),
            (Constants.INDIVIDUAL, {"name": "new_york"}, [Node(Constants.ITEM, {}, ["New York"])]),
            ]
    ...
    def assert_abstract_contains_function(self, expected):
        actual_functions_section = self._get_section(
            self.generator._abstract_gf_content.getvalue(), "fun")
    ...
    def assert_semantic_contains_lincat(self, expected):
        actual_lincat_section = self._get_section(self.generator._semantic_gf_content.getvalue(),
            start_line="lincat",
            end_line="lin")
...
class HeaderTestCase(AutoGeneratorTestCase):
    def test_header(self):
        ...
        self.assert_abstract_begins_with(
...
class CategoriesTestCase(AutoGeneratorTestCase):
    def test_sortal_category(self):
        ...
        self.given_grammar([])
        self.when_generating()
        self.assert_abstract_contains_category(
            "Sort_city;\n")
...
class ActionTestCase(AutoGeneratorTestCase):
    def test_action_base_case(self):
        self.given_grammar([
            Node(Constants.ACTION, {"name":"settings"},
                 [Node(Constants.ITEM, {}, [u"inställningar"])])])
        self.when_generating()
    ...
    def test_vp_action_as_single_variant(self):
        self.given_grammar([
                Node(Constants.ACTION, {"name": "set_time"},
                     [Node(Constants.ITEM, {},
                           [Node(Constants.VP, {}, [
    ...
    def test_np_action_with_explicit_definite(self):
        ...
        self.assert_semantic_contains_linearization(
            'settings = pp "settings";')
        self.assert_natural_language_contains_linearization(
            u'settings = (mkdef "inställningar" "inställningarna");')

    def test_np_action_with_implicit_definite(self):
        self.given_grammar([
            Node(Constants.ACTION, {"name": "settings"}, [
                Node(Constants.NP, {}, [
                    Node(Constants.INDEFINITE, {}, [u"preferences"])])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            'settings : NpAction;')
        self.assert_semantic_contains_linearization(
...
class PredicateTestCase(AutoGeneratorTestCase):
    def test_predicate_base_case(self):
        ...
        self.assert_semantic_contains_linearization(
            'price = pp "price";')
        self.assert_natural_language_contains_linearization(
            'price = ss (("price information"));')


class TitleTestCase(AutoGeneratorTestCase):
    def test_issue_title(self):
        self.given_grammar([
            Node(Constants.ISSUE_TITLE, {"predicate": "price"},
                [Node(Constants.ITEM, {}, ["Price Information"])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            'price_title : SysTitle;')
        self.assert_semantic_contains_linearization(
            'price_title = issueTitle price;')
        self.assert_natural_language_contains_linearization(
            'price_title = ss "Price Information";')

    def test_action_title(self):
        self.given_compulsory_grammar_for_app()
        self.given_grammar([
            Node(Constants.ACTION_TITLE, {"action": "top"},
                [Node(Constants.ITEM, {}, ["Main Menu"])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            'top_title : SysTitle;')
        self.assert_semantic_contains_linearization(
            'top_title = actionTitle (pp "top");')
        self.assert_natural_language_contains_linearization(
            'top_title = ss "Main Menu";')
...
class test_parameterized_sys_form_answer(AutoGeneratorTestCase):
    def test_in_middle(self):
        ...
        self.when_called()
        self.then_result_is(
            '"cannot set the hour to " ++ hour_to_set.alt ++ "."')
    ...
    def test_two_parameters(self):
        ...
        self.when_called()
        self.then_result_is(
            '"cannot set the time to " ++ hour_to_set.alt ++ " " ++ minute_to_set.alt')
    ...
    def test_two_parameters_switched_order(self):
        ...
        self.when_called()
        self.then_result_is(
            '"cannot set the time to " ++ minute_to_set.alt ++ " past " ++ hour_to_set.alt')
...
class ValidityTest(AutoGeneratorTestCase):
    ...
    def test_grammar_may_exclude_all_optional_parameter(self):
        ...
        self.given_grammar([
                Node(Constants.VALIDITY, {"name": "CityValidity"},
                     [Node(Constants.ITEM, {}, ["invalid parameters"])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            "CityValidity_1 : SysAnswer -> SysICM;")
        self.assert_abstract_contains_function(
        ...
        self.assert_semantic_contains_linearization(
            'CityValidity_2 dest_city = rejectICM (set (list dest_city)) "CityValidity";')
        self.assert_natural_language_contains_linearization(
            'CityValidity_1 dept_city = ss ("invalid parameters");')
        self.assert_natural_language_contains_linearization(
            'CityValidity_2 dest_city = ss ("invalid parameters")')
...
class ReportTestCase(AutoGeneratorTestCase):
    def test_multiple_parameters_base(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'report_ended_SetTime_1 : SysAnswer -> SysAnswer -> SysReportEnded;\n')
        self.assert_semantic_contains_linearization(
    ...
    def test_no_parameters(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'report_ended_SetTime_1 : SysReportEnded;\n')
        self.assert_semantic_contains_linearization(
            'report_ended_SetTime_1 = report_ended "SetTime" (empty_list);\n')
        self.assert_natural_language_contains_linearization(
            'report_ended_SetTime_1 = ss ("the time was set.");\n')
    ...
    def test_reason_and_no_parameters(self):
        self.given_grammar([
            Node(Constants.REPORT_FAILED, {"action": "Snooze", "reason": "not_ringing"},
                [Node(Constants.ITEM, {}, ["the alarm is not ringing."])])])
        self.given_service_interface_has_action(
            ServiceActionInterface("Snooze", DeviceModuleTarget(device="MockedDevice"), parameters=[], failure_reasons=[
                ActionFailureReason("not_ringing"),
            ]))
        self.given_generator()
        self.when_generating()
        self.assert_abstract_contains_function(
            'report_failed_Snooze_not_ringing_2 : SysReportFailed;\n')
        self.assert_semantic_contains_linearization(
    ...
    def test_reason_and_optional_parameter(self):
        self.given_grammar([
            Node(Constants.REPORT_FAILED, {"action": "SetTime", "reason": "invalid_time"},
                [Node(Constants.ONE_OF, {},
                      [Node(Constants.ITEM, {},
                       ...
                       Node(Constants.ITEM, {},
                            ...
                            "."])])])])
        self.given_service_interface_has_action(
            ServiceActionInterface("SetTime", DeviceModuleTarget(device="MockedDevice"), parameters=[
                ServiceParameter("hour_to_set"),
                ServiceParameter("minute_to_set", is_optional=True)
            ], failure_reasons=[
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'report_failed_SetTime_invalid_time_3 : SysAnswer -> SysReportFailed;\n')
        self.assert_abstract_contains_function(
    ...
    def test_unknown_failure_without_parameters(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'report_failed_Snooze_undefined_failure_1 : SysReportFailed;\n')
        self.assert_semantic_contains_linearization(
...
class VerbPhraseTest(AutoGeneratorTestCase):
    def test_verb_phrase(self):
        ...
        self.assert_natural_language_contains(
            'set_time = (mkverb "set" "set" "setting" "the time")')
...
class WarningsAndErrorsTest(AutoGeneratorTestCase):
    ...
    def test_generates_report_ended_if_entry_exists_even_without_postconfirmation(self):
        ...
        self.given_grammar([
                Node(Constants.REPORT_ENDED, {"action": "AlarmRings"},
                     [Node(Constants.ITEM, {}, ["The alarm stopped"])])])
        self.when_generating()
        self.expect_report_ended_was_generated_for_action("AlarmRings")

    def test_unexpected_parameter_name(self):
        self.given_service_interface_has_validity(
            ServiceValidatorInterface("HourValidity", self.mock_service_target, parameters=[
                ServiceParameter("hour_to_set")
            ]))
        self.given_grammar([
                Node(Constants.VALIDITY, {"name": "HourValidity"},
                     [Node(Constants.ITEM, {},
                           ["cannot set the hour to ",
                            Node(Constants.SLOT, {"predicate": "dest_city"}), "."])])])
    ...
    def test_warn_about_missing_entry_for_goal_issue_py_grammar(self):
        ...
        self.given_grammar([
                Node(Constants.USER_QUESTION, {"predicate": "price"},
                     [Node(Constants.ITEM, {}, ['what is the price'])])])
        self.when_generating()
        self.expect_warning(
    ...
    def test_not_warn_about_system_question_due_to_general_entry(self):
        ...
        self.given_compulsory_grammar_for_app()
        self.given_grammar([
                Node(Constants.PREDICATE, {"name": "dest_city"},
                     [Node(Constants.ITEM, {}, ["the destination"])])])
        self.when_generating()
        self.expect_no_warning()
    ...
    def test_background_with_string_answer_raises_exception(self):
        self.given_compulsory_grammar_for_app()
        self.given_grammar([
                Node(Constants.SYS_ANSWER, {"predicate": "location"},
                    [Node(Constants.ITEM, {}, [
        ...
        with self.assertRaises(InvalidSortOfBackgroundPredicateException) as e:
            self.when_generating()
        self.assertIn("Background is not allowed for predicate 'location' of sort 'string'. Perhaps you can create a new sort for it?", e.exception)

    def test_background_with_integer_answer_raises_exception(self):
        self.given_compulsory_grammar_for_app()
        self.given_grammar([
                Node(Constants.SYS_ANSWER, {"predicate": "frequent_flyer_points"},
                    [Node(Constants.ITEM, {}, [
                        Node(Constants.SLOT, {"predicate": "frequent_flyer_points"}),
                        " blablabla ",
                        Node(Constants.SLOT, {"predicate": "location"}),
        ...
        with self.assertRaises(InvalidSortOfBackgroundPredicateException) as e:
            self.when_generating()
        self.assertIn("Background is not allowed for predicate 'frequent_flyer_points' of sort 'integer'. Perhaps you can create a new sort for it?", e.exception)
    ...
    def expect_report_ended_was_generated_for_action(self, action_name):
        self.assert_abstract_contains_function(
            'report_ended_AlarmRings_1 : SysReportEnded;\n')
...
class SysAnswerTest(AutoGeneratorTestCase):
    ...
    def test_nullary_propositional_system_answer(self):
        ...
        self.when_generating_system_answer_content(
    ...
    def test_overridden_integer_system_answer(self):
        self.given_grammar([
                Node(Constants.SYS_ANSWER, {"predicate": "distance"},
                     [Node(Constants.ITEM, {},
                           ["the distance is ",
                            Node(Constants.SLOT, {}),
                            "meters"])])])
    ...
    def when_generating_system_answer_content(self, predicate_name):
        self.generator._grammar = self._grammar
        self.generator._generate_system_answer_content(
    ...
    def test_overridden_string_system_answer(self):
        self.given_grammar([
                Node(Constants.SYS_ANSWER, {"predicate": "location"},
                    [Node(Constants.ITEM, {},
                          [u"du är på ", Node(Constants.SLOT, {})])])])
        self.given_generator()
        self.when_generating_system_answer_content("location")
        self.assert_abstract_contains("location_sys_answer_0 : SysAnswer;\n")
        self.assert_semantic_contains('location_sys_answer_0 = pp "location" string_placeholder_0;\n')
    ...
    def test_default_datetime_system_answer(self):
        ...
        self.assert_abstract_contains(
            "flight_departure_sys_answer_0 : SysAnswer;\n")
        self.assert_semantic_contains(
            'flight_departure_sys_answer_0 = pp "flight_departure" datetime_placeholder_0;\n')
        self.assert_natural_language_contains(
            'flight_departure_sys_answer_0 = answer ("_datetime_placeholder_0_");\n')

    def test_overridden_datetime_system_answer(self):
        self.given_grammar([
            Node(Constants.SYS_ANSWER, {"predicate": "flight_departure"},
                 [Node(Constants.ITEM, {},
                       [u"The flight departs at", Node(Constants.SLOT, {})])])])
        self.given_generator()
        self.when_generating_system_answer_content("flight_departure")
        self.assert_abstract_contains(
            "flight_departure_sys_answer_0 : SysAnswer;\n")
...
class UserAnswerTest(AutoGeneratorTestCase):
    ...
    def test_no_boolean_answer_content(self):
        self.given_grammar([])
        self.when_generating()
        self.assert_abstract_not_contains_function(
            "boolean_user_answer : Sort_boolean -> UsrAnswer;\n")
        self.assert_semantic_not_contains_linearization(
            "boolean_user_answer answer = answer;\n")
        self.assert_natural_language_not_contains_linearization(
            "boolean_user_answer answer = answer;\n")
    ...
    def test_propositional_user_answer_for_custom_sort(self):
        ...
        self.assert_natural_language_contains_linearization(
            'dest_city_propositional_usr_answer answer = answer;\n')

    def test_propositional_user_answer_for_string_sort(self):
        self.given_grammar([])
        self.when_generating()
        self.assert_abstract_contains_function(
            'comment_propositional_usr_answer : Unknown -> Predicate_comment;\n')
        self.assert_semantic_contains_linearization(
            'comment_propositional_usr_answer answer = pp comment.s (ss ("\\"" ++ answer.s ++ "\\""));\n')
        self.assert_natural_language_contains_linearization(
            'comment_propositional_usr_answer answer = answer;\n')
    ...
    def assert_generated_gf_contains_sort_user_answer_with_sort_string(self):
        self.assert_abstract_contains_function(
            "comment_sortal_usr_answer : Sort_string -> UsrAnswer;\n")
        self.assert_semantic_contains_linearization(
            "comment_sortal_usr_answer answer = answer;\n")
        self.assert_natural_language_contains_linearization(
            'comment_sortal_usr_answer answer = answer;\n')
...
class UserQuestionTest(AutoGeneratorTestCase):
    def test_without_answers(self):
        ...
        self.assert_abstract_contains_function(
            "ask_price : UsrWHQ;\n")
        self.assert_semantic_contains_linearization(
            "ask_price = ask_whq price;\n")
        self.assert_natural_language_contains_linearization(
            'ask_price = ss (("price information"));\n')

    def test_with_sortal_answer_of_custom_sort(self):
        self.given_grammar([
            Node(Constants.USER_QUESTION, {"predicate": "price"},
                 [Node(Constants.ITEM, {},
                       ["price to ", Node(Constants.SLOT, {"sort": "city"})])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            'price_user_question_1 : Sort_city -> UsrWHQ;\n')
        self.assert_semantic_contains_linearization(
            "price_user_question_1 city = ask_whq price city;\n")
        self.assert_natural_language_contains_linearization(
    ...
    def test_with_propositional_answer_of_custom_sort(self):
        self.given_grammar([
            Node(Constants.USER_QUESTION, {"predicate": "price"},
                 [Node(Constants.ITEM, {},
                       ["price to ", Node(Constants.SLOT, {"predicate": "dest_city"})])])])
        self.when_generating()
        self.assert_abstract_contains_function(
            "price_user_question_1 : Predicate_dest_city -> UsrWHQ;\n")
        self.assert_semantic_contains_linearization(
            "price_user_question_1 dest_city = ask_whq price dest_city;\n")
        self.assert_natural_language_contains_linearization(
            'price_user_question_1 dest_city = ss ("price to " ++ dest_city.s);\n')

    def test_with_propositional_literal_answer(self):
        self.given_grammar([
            Node(Constants.USER_QUESTION, {"predicate": "attraction_information"},
                 [Node(Constants.ITEM, {}, ["tell me about ",
                                            Node(Constants.SLOT, {"predicate": "attraction"})])])])
        self.given_generator()
        self.when_generating()
        self.assert_abstract_contains_function(
...
class SysQuestionTest(AutoGeneratorTestCase):
    ...
    def when_generating_system_question(self, predicate_name):
        self.generator._generate_system_question(
            self.get_predicate(predicate_name), self._form)
...
class BackgroundEmbeddingTest(AutoGeneratorTestCase):
    def test_single_background_for_wh_question(self):
        self.given_form(
            Node(Constants.ITEM, {},
                 ["which hour ",
                  Node(Constants.SLOT, {"predicate": "day_to_set"})]))
        self.given_generator()
        self.when_generating_predicate_content("hour_to_set")
        self.assert_abstract_contains(
            'hour_to_set_ask_whq_with_background_1 : SysAnswer -> System;\n')
        self.assert_semantic_contains(
...
class SysResolveYnqTest(AutoGeneratorTestCase):
    def test_base_case_no_background(self):
        ...
        self.assert_abstract_contains(
            'price_resolve_ynq_1 : SysResolveGoal;\n')
        self.assert_semantic_contains(
            'price_resolve_ynq_1 = resolve_ynq price;\n')
        self.assert_natural_language_contains(
            'price_resolve_ynq_1 = resolve_ynq price;\n')
    ...
    def test_single_background(self):
        ...
        self.when_generating_predicate_content("price")
        self.assert_abstract_contains(
            'price_resolve_ynq_2 : SysAnswer -> SysResolveGoal;\n')
        self.assert_semantic_contains(
...
class GreetingTest(AutoGeneratorTestCase):
    def test_greeting(self):
        ...
        self.when_generating()
        self.assert_natural_language_contains_linearization(
            'sysGreet = ss "Welcome to the Service!";')
    ...
    def test_greeting_contains_unicode(self):
        self.given_grammar([
                Node(Constants.GREETING, {}, [Node(Constants.ITEM, {}, [u"Hallå!"])])])
        self.given_generator()
        self.when_generating()
        self.assert_natural_language_contains_linearization(
            u'sysGreet = ss "Hallå!";')
...
class IndividualTest(AutoGeneratorTestCase):
    def test_base_case(self):
        self.given_grammar([
                Node(Constants.INDIVIDUAL, {"name": "new_york"}, [Node(Constants.ITEM, {}, ["New York"])])])
        self.given_generator()
        self.when_generating()
        self.assert_abstract_contains_function(
            'new_york : Sort_city;\n')
    ...
    def test_placeholders_for_dynamic_sorts(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'placeholder_keyword0 : Sort_keyword;\n')
        self.assert_semantic_contains_linearization(
            'placeholder_keyword0 = pp "_sem_placeholder_keyword0_";\n')
        self.assert_natural_language_contains_linearization(
            'placeholder_keyword0 = ss "_nl_placeholder_keyword0_";\n')
...
class PlaceholdersForBuiltinSortsTestCase(AutoGeneratorTestCase):
    def test_integer(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'integer_placeholder_0 : Integer;\n')
        self.assert_semantic_contains_linearization(
            'integer_placeholder_0 = pp "_integer_placeholder_0_";\n')
        self.assert_natural_language_contains_linearization(
    ...
    def test_datetime(self):
        ...
        self.when_generating()
        self.assert_abstract_contains_function(
            'datetime_placeholder_0 : DateTime;\n')
        self.assert_semantic_contains_linearization(
            'datetime_placeholder_0 = pp "_datetime_placeholder_0_";\n')
        self.assert_natural_language_contains_linearization(
            'datetime_placeholder_0 = ss "_datetime_placeholder_0_";\n')
...
class LoadingAndCompilationTestCase(AutoGeneratorTestCase):
    ...
    def test_load_and_compile_grammar_entries_from_xml(self):
        self._given_grammar_source_file(
            "grammar_eng.xml",
            '<grammar><action name="top">start view</action></grammar>')
        self._when_load_and_compile_grammar_entries("eng")
        self._then_result_is(
            Node(Constants.GRAMMAR, {}, [
                Node(Constants.ACTION, {"name": "top"}, [
    ...
    def test_validate_invalid_xml_grammar(self):
        with self.assertRaises(ViolatesSchemaException) as context:
            self._given_grammar_source_file(
                "grammar_eng.xml",
                '<grammar><invalid_element name="top">start view</invalid_element></grammar>')
            self._when_load_and_compile_grammar_entries("eng")
        self.assertIn("Expected grammar_eng.xml compliant with schema but it's in violation: "
